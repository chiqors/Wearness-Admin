...
<div class="col-6 col-md-8">
    <h1><span>Feed back</span> Table</h1>
    <p>Hi {{ Auth::user()->name }}, this is Feed back table</p>
</div>
...
<div class="modal fade text-dark" id="modalFeedback" tabindex="-1" role="dialog" aria-labelledby="modalAddLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
...
<script type="text/javascript">    
    ...
    $(document).ready(function() {
      const dataTable = $('.dataTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{!! route("feedback.create") !!}',
        columns: [
        { data: 'DT_RowIndex',name: 'DT_RowIndex' ,orderable: false,searchable: false, editable: true },
        { data: 'category_feedback.category', name: 'category_feedback.category' },                
        { data: 'feedback', 
          name: 'feedback',
          render: function (data,type , row) {                                     
            return "<button class='btn btn-primary' onClick='feedbackShow(\"" + row.id + "\",\"" + data + "\")' data-toggle='modal' data-target='#modalFeedback'>File</button>";
          }
        },                
    ...
    $('.delete').on('click', (e) => {
      ...
      ajax('/feedback/'+id[0], ids, 'PUT', dataTable)
