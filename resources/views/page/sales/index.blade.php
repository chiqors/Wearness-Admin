...
<div class="col-6 col-md-8">
    <h1><span>Sales</span> Table</h1>
    <p>Hi {{ Auth::user()->name }}, this is sales table</p>
</div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalEditLabel">Add sales</h5>
                    ...
                    <div class="row">
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for=""><span class="font-weight-400">Buyer</span></label>
                                    <input type="text" class="form-control" name="buyer" placeholder="Write a buyer" required>
                                </div>
                            </div>                      
                            <div class="col-md-12">
                                <div class="form-group">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                  <label for=""><span class="font-weight-400">Device / Serial number</span></label>
                                  <select name="device" id="" class="form-control" required>
                                    @foreach ($device as $key => $value)
                                      <option value="{{ $value->id }}">{{ $value->device_name .'/'. $value->serial_number }}</option>
                                    @endforeach
                                  </select>
                                </div>  
                            </div>                              
                            <div class="col-md-12">
                                <label for=""><span class="font-weight-400">Date</span></label>
...
<script type="text/javascript">    
    $(document).ready(function() {
      const dataTable = $('.dataTable').DataTable({
        ...
        select: true,
         dom: "<'row'<'col-sm-12 col-md-4'><'col-sm-12 col-md-2'l><'col-sm-12 col-md-6'f>>"
               +'rt'+
    ...
    $('#modalAdd').on('hide.bs.modal', function (e) {
      ...
      if($('#addForm').valid()){     
        const data = {
            ...
            device: $('[name=device] :selected').val(),
            date: $('[name=fromdateEdit]').val(),
        }
        ajax('/sales', data, null, dataTable)
        $('#modalAdd').modal('hide')
      }
      })
    ...
    $('.delete').on('click', (e) => {
      ...
      ajax('/sales/'+id[0], ids, 'PUT', dataTable)
